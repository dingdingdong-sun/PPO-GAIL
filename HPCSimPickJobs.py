...
class HPCEnv(src.gym.gym.Env):
    ...
    def my_init(self, workload_file = '', sched_file = ''):
        ...
        self.cluster = Cluster("Cluster", self.loads.max_nodes,
                               10, self.loads.max_requested_memory)
        self.action_space = spaces.Discrete(MAX_QUEUE_SIZE)
        self.observation_space = spaces.Box(low=0.0, high=1.0,
                                            shape=(JOB_FEATURES * MAX_QUEUE_SIZE + self.loads.max_nodes,),
                                            dtype=np.float32)
    ...
    def build_observation(self):
        '返回观测到地队列中的任务的状态'
        vector = np.zeros((MAX_QUEUE_SIZE) * JOB_FEATURES + self.loads.max_nodes, dtype=float)
