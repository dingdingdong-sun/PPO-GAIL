...
def critic_mlp(x, act_dim, cluster_node_num):
    # job_f = x[:MAX_QUEUE_SIZE * JOB_FEATURES]
    # cluster_f = x[MAX_QUEUE_SIZE * JOB_FEATURES:]
    job_f, cluster_f = tf.split(x, [MAX_QUEUE_SIZE * JOB_FEATURES, cluster_node_num], axis=1)
    x = tf.reshape(job_f, shape=[-1, MAX_QUEUE_SIZE, JOB_FEATURES])
    ...
    y = tf.reshape(cluster_f, shape=[-1, cluster_node_num, 1])
    y = tf.layers.dense(y, units=32, activation=tf.nn.relu)
    y = tf.layers.dense(y, units=16, activation=tf.nn.relu)
    y = tf.layers.dense(y, units=8, activation=tf.nn.relu)
    y = tf.layers.dense(y, units=1)

    x = tf.squeeze(tf.concat([x, y], axis=1), axis=-1)
...
def rl_kernel(x, act_dim, cluster_node_num):
    print(tf.shape(x))
    job_f, cluster_f = tf.split(x, [MAX_QUEUE_SIZE * JOB_FEATURES, cluster_node_num], axis=1)
    x = tf.reshape(job_f, shape=[-1, MAX_QUEUE_SIZE, JOB_FEATURES])
    x = tf.layers.dense(x, units=32, activation=tf.nn.relu)
...
def categorical_policy(x, a, mask, action_space, attn, cluster_node_num):
    act_dim = action_space.n
    if attn:
        output_layer = attention(x, act_dim)
    else:
        output_layer = rl_kernel(x, act_dim, cluster_node_num)
...
def actor_critic(x, a, mask, cluster_node_num, action_space=None, attn=False):
    with tf.variable_scope('pi'):
        pi, logp, logp_pi, out = categorical_policy(x, a, mask, action_space, attn, cluster_node_num)
    with tf.variable_scope('v'):
        v = tf.squeeze(critic_mlp(x, 1, cluster_node_num), axis=1)
...
def ppo(workload_file, model_path, ac_kwargs=dict(), seed=0,
        traj_per_epoch=400, epochs=50, gamma=0.99, clip_ratio=0.2, pi_lr=3e-4,
    ...
    if pre_trained:
        ...
        pi, logp, logp_pi, v, out = actor_critic(x_ph, a_ph, mask_ph, len(env.cluster.node_list), **ac_kwargs)
