...
class HPCEnv(src.gym.gym.Env):
    def __init__(self, shuffle=False, backfil=False, skip=False, job_score_type=0, batch_job_slice=0,
        ...
        self.sjf_scores = []
        self.job_wait_ratio = 0

    # @profile
    def my_init(self, workload_file='', sched_file=''):
        print("loading workloads from dataset:", workload_file)
        self.loads = [Workloads(file) for file in workload_file.strip().split()]
        self.max_nodes = 0
        self.max_mem = 0
        ...
        for load in self.loads:
            self.max_nodes = max(self.max_nodes, load.max_nodes)
            self.max_mem = max(self.max_mem, load.max_requested_memory)
            self.max_exec_time = max(self.max_exec_time, load.max_exec_time)
            self.max_proc = max(self.max_proc, load.max_procs)

        for load in self.loads:
            load.resize(self.max_proc, math.ceil(self.max_proc/float(self.max_nodes)))
        ...
        self.cluster = Cluster("Cluster", self.max_nodes, math.ceil(self.max_proc/float(self.max_nodes)), self.max_mem)
        self.action_space = spaces.Discrete(MAX_QUEUE_SIZE)
        self.observation_space = spaces.Box(low=0.0, high=1.0,
                                            shape=(
                                            JOB_FEATURES * MAX_QUEUE_SIZE,),
                                            dtype=np.float32)
        self.penalty_job_score = JOB_SEQUENCE_SIZE * self.max_exec_time / 10
        ...
        if self.build_sjf:  # this is for trajectory filtering.
            ...
            if "after_filter_traj_start.txt" in glob.glob(r"./*"):
                ...
                if self.batch_job_slice == 0:
                    max_index = self.loads[self.start_index].size() - JOB_SEQUENCE_SIZE - 1
                else:
                    max_index = min(self.batch_job_slice, self.loads[self.start_index].size()) - JOB_SEQUENCE_SIZE - 1
                ...
                while index <= max_index:
                    ...
                    self.current_timestamp = self.loads[self.load_index][self.start].submit_time
                    self.job_queue.append(self.loads[self.load_index][self.start])
    ...
    def gen_preworkloads(self, size):
        ...
        for i in range(running_job_size):
            _job = self.loads[self.load_index][self.start - i - 1]
    ...
    def reset(self):
        ...
        self.load_index = random.randint(0, len(self.loads)-1)
        self.job_queue = []
        self.running_jobs = []
        self.visible_jobs = []
        self.pairs = []
        ...
        if self.build_sjf:
            ...
            if self.batch_job_slice == 0:
                self.start = self.np_random.randint(job_sequence_size, (self.loads[self.load_index].size() - job_sequence_size - 1))
            else:
                self.start = self.np_random.randint(job_sequence_size, (self.batch_job_slice - job_sequence_size - 1))

        print("loads index: {}".format(self.load_index))
    ...
    def skip_for_resources_greedy(self, job, scheduled_logs):
        ...
        while not self.cluster.can_allocated(job):
            ...
            if self.next_arriving_job_idx < self.last_job_in_batch \
                    and self.loads[self.load_index][self.next_arriving_job_idx].submit_time <= next_resource_release_time:
                self.current_timestamp = max(self.current_timestamp, self.loads[self.load_index][self.next_arriving_job_idx].submit_time)
                self.job_queue.append(self.loads[self.load_index][self.next_arriving_job_idx])
    ...
    def post_process_score(self, scheduled_logs):
        if self.job_score_type == 0:
            ...
            total_cpu_hour = (self.current_timestamp - self.loads[self.load_index][self.start].submit_time) * self.loads.max_procs
            for i in scheduled_logs:
    ...
    def schedule_curr_sequence_reset(self, score_fn):
        ...
        self.cluster.reset()
        self.loads[self.load_index].reset()
        self.job_queue = []
        self.running_jobs = []
        self.visible_jobs = []
        self.pairs = []
    ...
    def build_critic_observation(self):
        ...
        earlist_job = self.loads[self.load_index][self.start_idx_last_reset]
        earlist_submit_time = earlist_job.submit_time
        pairs = []
        for i in range(self.start_idx_last_reset, self.last_job_in_batch + 1):
            job = self.loads[self.load_index][i]
            submit_time = job.submit_time - earlist_submit_time
            request_processors = job.request_number_of_processors
            request_time = job.request_time

            normalized_submit_time = min(float(submit_time) / float(MAX_WAIT_TIME), 1.0 - 1e-5)
            normalized_run_time = min(float(request_time) / float(self.loads[self.load_index].max_exec_time), 1.0 - 1e-5)
            normalized_request_nodes = min(float(request_processors) / float(self.loads[self.load_index].max_procs), 1.0 - 1e-5)
    ...
    def build_observation(self):
        '返回观测到地队列中的任务的状态'
        vector = np.zeros((MAX_QUEUE_SIZE) * JOB_FEATURES + self.max_nodes, dtype=float)
        ...
        if len(self.job_queue) <= MAX_QUEUE_SIZE:
            ...
            self.job_queue.sort(key=lambda job: self.fcfs_score(job))
            for i in range(0, MAX_QUEUE_SIZE):
                visible_fcfs.append(self.job_queue[i])
            ...
            while index < MAX_QUEUE_SIZE:
                ...
                # sjf_index += 1
                fcfs_job = visible_fcfs[fcfs_index]
                fcfs_index += 1
                ...
                #     index += 1
                if (not fcfs_job in self.visible_jobs) and index < MAX_QUEUE_SIZE:
                    self.visible_jobs.append(fcfs_job)
                    index += 1
        ...
        for i in range(0, MAX_QUEUE_SIZE):
            if i < len(self.visible_jobs) and i < MAX_QUEUE_SIZE:
                ...
                if job.request_memory == -1:
                    ...
                    normalized_request_memory = min(float(job.request_memory) / float(self.loads[self.load_index].max_requested_memory),
                                                    1.0 - 1e-5)

                if job.user_id == -1:
                    normalized_user_id = 1
                else:
                    normalized_user_id = min(float(job.user_id)/float(self.loads[self.load_index].max_user_id), 1.0-1e-5)

                if job.group_id == -1:
                    normalized_group_id = 1
                else:
                    normalized_group_id = min(float(job.group_id) / float(self.loads[self.load_index].max_group_id), 1.0 - 1e-5)

                if job.executable_number == -1:
                    normalized_executable_id = 1
                else:
                    normalized_executable_id = min(
                        float(job.executable_number) / float(self.loads[self.load_index].max_executable_number), 1.0 - 1e-5)
        ...
        for job in self.running_jobs:
            for m in job.allocated_machines.keys():
                ...
                vector[MAX_QUEUE_SIZE * JOB_FEATURES + machine_index] += \
    ...
    def moveforward_for_resources_backfill(self, job):
        ...
        self.running_jobs.sort(key=lambda running_job: (running_job.scheduled_time + running_job.request_time))
        free_nodes = 0
        for key in self.cluster.all_nodes.keys():
            if key >= request_procs_per_node_job:
                free_nodes += len(self.cluster.all_nodes[key])
        for running_job in self.running_jobs:
            for m in running_job.allocated_machines.keys():
                if m.now_proc < request_procs_per_node_job and m.now_proc+running_job.allocated_machines[m] >= request_procs_per_node_job:
                    free_nodes += 1
                m.now_proc += running_job.allocated_machines[m]
            ...
            if free_nodes >= job.request_number_of_nodes:
                for m in self.cluster.node_list:
                    m.now_proc = m.processor
                break
        ...
        while not self.cluster.can_allocated(job):
            ...
            if self.next_arriving_job_idx < self.last_job_in_batch and \
                    self.loads[self.load_index][self.next_arriving_job_idx].submit_time <= next_resource_release_time:
                self.current_timestamp = max(self.current_timestamp, self.loads[self.load_index][self.next_arriving_job_idx].submit_time)
                self.job_queue.append(self.loads[self.load_index][self.next_arriving_job_idx])
                self.next_arriving_job_idx += 1
            else:
    ...
    def moveforward_for_job(self):
        ...
        while not self.job_queue:
            ...
            if self.loads[self.load_index][self.next_arriving_job_idx].submit_time <= next_resource_release_time:
                self.current_timestamp = max(self.current_timestamp, self.loads[self.load_index][self.next_arriving_job_idx].submit_time)
                self.job_queue.append(self.loads[self.load_index][self.next_arriving_job_idx])
                self.next_arriving_job_idx += 1
    ...
    def skip_schedule(self):
        ...
        if next_time_after_skip < min(self.loads[self.load_index][self.next_arriving_job_idx].submit_time, next_resource_release_time):
            self.current_timestamp = next_time_after_skip
            return False, 0

        if self.next_arriving_job_idx < self.last_job_in_batch and self.loads[self.load_index][
