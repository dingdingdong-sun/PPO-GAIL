...
class Workloads:
    ...
    def __init__(self, path):
        ...
        #     job.request_number_of_nodes = job.request_number_of_nodes
        #     if job.user_id not in self.users_id.keys():
        #         self.users_id[job.user_id] = user_index
        #         user_index += 1

    def size(self):
        return len(self.all_jobs)

    def resize(self, max_proc, proc_per_node):
        for job in self.all_jobs:
            job.request_number_of_processors = math.ceil((max_proc / self.max_procs) * job.request_number_of_processors)
            job.request_number_of_nodes = math.ceil(job.request_number_of_processors / float(proc_per_node))
